...
func TestDeleteNamespace(t *testing.T) {
	...
	t.Run("NamespaceDelete", func(t *testing.T) {
		...
		json.NewEncoder(stdin).Encode(req)
		if !scanner.Scan() {
			t.Fatalf("no response for namespace_delete")
		}
		...
		if resp.Error != nil {
			t.Fatalf("namespace_delete error: %v", resp.Error.Message)
		}
	})

	// -------------------------------------------------
	// 4. Get namespace again (must return error)
	// -------------------------------------------------
	t.Run("NamespaceGetShouldFail", func(t *testing.T) {
		req := JSONRPCRequest{
			JSONRPC: "2.0",
			ID:      4,
			Method:  "tools/call",
		...
		if resp.Error == nil {
			t.Fatalf("expected error because namespace should be deleted, but got: %+v", resp.Result)
		}
	})
